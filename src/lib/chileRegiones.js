// Regiones y comunas de Chile (división administrativa vigente, 16 regiones)
...
export function comunasDeRegion(region) {
  return REGIONES_CHILE.find((r) => r.region === region)?.comunas || []
}

export function regionDeComuna(comuna) {
  return REGIONES_CHILE.find((r) => r.comunas.includes(comuna))?.region || ''
}
