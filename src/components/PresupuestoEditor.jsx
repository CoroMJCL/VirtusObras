  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="space-y-6 rounded-2xl border border-[#e5e5e7] bg-white p-7">
        <div>
          <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Cliente</label>
          <select
          ...
          >
            ...
            {clientes.map((c) => (
              <option key={c.id} value={c.id} className="bg-white">{c.nombre} {c.telefono ? `· ${c.telefono}` : ''}</option>
            ))}
          </select>
        </div>

        <div>
          <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Tipo de trabajo</label>
        ...
        <div>
          <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Descripción del trabajo</label>
          <textarea
            rows={3} value={presupuesto.descripcion || ''}
            onChange={(e) => setPresupuesto({ ...presupuesto, descripcion: e.target.value })}
            className="focus-ring w-full resize-none rounded-xl border border-[#d8d8dc] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/10 px-4 py-2.5 text-sm text-[#1d1d1f]"
          />
        </div>

        <div>
          <div className="mb-2 flex items-center justify-between">
            <label className="block text-[13px] font-medium text-[#1d1d1f]">Ítems</label>
            ...
            {presupuesto.items.map((item, idx) => (
              <div key={item.id} className="grid grid-cols-12 gap-2">
                <input
                  ...
                  className="focus-ring col-span-6 rounded-lg border border-[#d8d8dc] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/10 px-3 py-2 text-sm text-[#1d1d1f]"
                />
                <input
                  type="number" min="0" placeholder="Cant." value={item.cantidad}
                  onChange={(e) => actualizarItem(idx, 'cantidad', e.target.value)}
                  className="focus-ring col-span-2 rounded-lg border border-[#d8d8dc] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/10 px-3 py-2 text-sm text-[#1d1d1f]"
                />
                <input
                  ...
                  className="focus-ring col-span-3 rounded-lg border border-[#d8d8dc] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/10 px-3 py-2 text-sm text-[#1d1d1f]"
                />
                <button onClick={() => quitarItem(idx)} className="focus-ring col-span-1 flex items-center justify-center text-[#86868b] hover:text-red-600">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
        ...
        <div>
          <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Excepciones</label>
          <textarea
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Fecha de creación</label>
            <input
              type="date"
              ...
              className="focus-ring w-full rounded-xl border border-[#d8d8dc] bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] focus:border-[#c9a227] focus:ring-2 focus:ring-[#c9a227]/10 px-4 py-2.5 text-sm text-[#1d1d1f] [color-scheme:dark]"
            />
          </div>
          <div>
            <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Descuento (CLP)</label>
            <input
          ...
          <div>
            <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Validez (días)</label>
            <input
        ...
        <div>
          <label className="mb-2 block text-[13px] font-medium text-[#1d1d1f]">Etapa del expediente</label>
          <div className="flex gap-2">
