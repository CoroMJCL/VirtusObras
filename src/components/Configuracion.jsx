...
const inputClass = "focus-ring w-full rounded-xl border border-[#d8d8dc] bg-white px-4 py-2.5 text-[14px] text-[#1d1d1f] shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition-colors focus:border-[#c9a227] focus:outline-none focus:ring-2 focus:ring-[#c9a227]/10"
const labelClass = "mb-1.5 block text-[13px] font-medium text-[#1d1d1f]"
const helpClass = "mt-1.5 text-[12.5px] text-[#a0a0a5]"

function Seccion({ titulo, descripcion, children }) {
  return (
    <div className="rounded-2xl border border-[#e5e5e7] bg-white p-7 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
      <div className="mb-6">
        <h2 className="text-[16px] font-semibold tracking-tight text-[#1d1d1f]">{titulo}</h2>
        {descripcion && <p className="mt-0.5 text-[13px] text-[#86868b]">{descripcion}</p>}
      </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-8 flex items-center justify-between">
        <h1 className="text-[26px] font-medium tracking-tight text-[#1d1d1f]">Configuración</h1>
        {guardado && (
      ...
      <form onSubmit={guardar} className="space-y-6">
        ...
        <button
          type="submit"
          disabled={guardando}
          className="focus-ring w-full rounded-full bg-gold-gradient py-3 text-[14px] font-medium text-[#1d1d1f] shadow-[0_4px_14px_rgba(201,162,39,0.3)] transition-opacity disabled:opacity-50"
        >
          {guardando ? 'Guardando…' : 'Guardar cambios'}
        </button>
      </form>

      <p className="mt-6 text-[12.5px] leading-relaxed text-[#a0a0a5]">
